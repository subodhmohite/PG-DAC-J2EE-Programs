...
public class LoginServlet extends HttpServlet {
	...
	/* overriding form of method cant throw NEW or BROADER checked excp 
	 * */
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter pw = response.getWriter()) {
			...
			if (user == null) {
				...
				//valid login--1.Create a Cookie to store validated user
				//javax.servlet.http.Cookie(String cookieName,String cookie
				Cookie c1=new Cookie("user_details",user.toString());
				response.addCookie(c1);
				//2.Send the cookie to client 
...
//				pw.print("<h5> User Details "+user+"</h5>");
				if (user.getRole().equals("voter")) {
					 ...
					 * resp body -empty
					 * 3. Client browser -checks privacy settings
					 * cookies blocked --cookies won't be stored --can't remember
